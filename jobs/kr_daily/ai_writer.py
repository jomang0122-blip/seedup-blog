...
def _md_to_html(text: str) -> str:
    """마크다운 표·제목·강조를 HTML로 변환. 기존 인라인 HTML은 유지."""
    try:
...
def _build_prompt(data: dict) -> str:
    ...
    investor_skip_note = (
        "\n⚠️ 수급 데이터 없음 → [메이저 수급 현황] 테이블 전체 생략할 것."
        if not investor_top3
        else ""
    )
...
3. 색상 규칙 (모든 등락률 수치에 예외 없이 적용):
   - 플러스(상승): <font color="red"><b>+X.XX%</b></font>
   - 마이너스(하락): <font color="blue"><b>-X.XX%</b></font>
4. TITLE: 순수 텍스트만. HTML 태그 절대 금지.
5. ## ### #### 제목: 검정 텍스트만. color 태그 금지.
6. 이유 출처: 뉴스 헤드라인에 있는 팩트만. 없으면 이유 생략.
7. 수급 데이터가 "(없음)"인 행은 생략. 전체 없으면 수급 테이블 전체 생략.
...
| 📈 상승 특징주 | 📉 하락 특징주 |
| :--- | :--- |
| **[종목A] (<font color="red"><b>+X.XX%</b></font>)**<br>— [뉴스 기반 이유] | **[종목X] (<font color="blue"><b>-X.XX%</b></font>)**<br>— [뉴스 기반 이유] |
| **[종목B] (<font color="red"><b>+X.XX%</b></font>)**<br>— [이유] | **[종목Y] (<font color="blue"><b>-X.XX%</b></font>)**<br>— [이유] |
...
> ⚠️ **면책 조항**: 본 포스팅은 공시 데이터 및 시장 뉴스를 바탕으로 작성된 단순 정보 제공 목적의 글이며, 특정 종목에 대한 매수 또는 매도 추천이 아닙니다. 모든 투자에 대한 판단과 책임은 투자자 본인에게 있습니다.

출력 형식 — 아래 3줄 헤더 뒤에 마크다운 본문만 작성 (다른 설명 없음):
