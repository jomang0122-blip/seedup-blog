...
def _build_dart_anchor(data: dict) -> str:
    """오늘 특징주 종목 중 임팩트 큰 DART 공시가 있는 종목만 표기.
    보고서명·접수일은 DART 응답 값 그대로만 허용(창작 금지). news 필드가
    있으면(관련 뉴스 매칭 성공) 그 뉴스 내용을 짧게 요약해 붙이는 것만
    허용 — DART list.json은 공시 목록만 주고 본문을 안 주므로, 검증된
    뉴스 검색으로 보강한 내용이다. 뉴스가 없으면 공시 사실(제목)만 표기."""
    disclosures = data.get("dart_disclosures", {})
    if not disclosures:
        return "(공시 없음 — #### 📋 관련 공시 소제목 포함 해당 하위 섹션 전체 생략. 텍스트 한 줄도 출력 금지.)"
    lines = []
    for name, info in disclosures.items():
        line = f"{name} | {info['report_nm']} | 접수 {info['rcept_dt']} | {info['url']}"
        if info.get("news"):
            line += f" | [관련뉴스: {info['news']}]"
        lines.append(line)
    return (
        "이 목록에 있는 종목·보고서명·접수일·URL만 사용 — 창작 금지. "
        "[관련뉴스]가 붙은 항목만 그 뉴스 내용을 짧은 명사형 구로 요약해 "
        "공시 내용 설명에 사용 가능(뉴스에 없는 내용 추가 금지). [관련뉴스]가 "
        "없는 항목은 보고서명(공시 제목)만 표기하고 내용 추측 금지:\n" + "\n".join(lines)
    )
...
def _build_prompt(data: dict, prev_issues: list = None) -> str:
    ...
    dart_anchor = _build_dart_anchor(data)
...
[오늘 특징주 종목의 관련 공시(DART) — 사실관계만 표기, 이유·해석 문장 작성 금지]
{dart_anchor}
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
6. 특징주 섹션별 이유 작성 규칙:
   ...
   - 📋 관련 공시: 데이터 블록 'DART' 목록에 있는 종목·보고서명·접수일·URL만 사용.
     "**[종목명]** [보고서명] (접수 [YYYY-MM-DD]) [[원문보기]](데이터 블록의 URL 그대로)"
     형식의 마크다운 링크 뒤에, [관련뉴스]가 붙은 항목만 그 뉴스를 짧은 명사형
     구로 요약해 " — " 뒤에 이어 쓸 것(예: "OCI홀딩스 잠정실적(공정공시) (접수
     2026-07-23) [원문보기] — 2분기 영업이익 1,081억 흑자전환"). [관련뉴스]가
     없는 항목은 뒤에 아무것도 붙이지 말고 공시 제목까지만. 뉴스에 없는 내용을
     추측해서 채우지 말 것 — 사실 나열만. URL은 데이터 블록 값을 한 글자도
     바꾸지 말 것. 목록이 없으면 이 하위 섹션 전체 생략.
   - 공통 금지 사항:
...
12. 레이아웃 구조 고정: 반드시 ### 섹션 3개만(📊 1. / 💥 2. / 🔮 3.) 유지. 섹션 추가·삭제·번호 변경·재배치 절대 금지.
    ...
    - #### 하위 섹션(상승/하락/뉴스기반 특징주·시가총액 상위 종목·관련 공시)은 아래
      [출력 레이아웃]에 제시된 순서를 그대로 지킬 것 — 임의로 순서를 바꾸거나 새 하위
      섹션을 추가하지 말 것. 특히 '관련 공시'는 반드시 마지막 하위 섹션에 위치.
    - 데이터 없어도 섹션 구조는 유지.
...
⚠️ 반드시 이 순서대로 마지막 하위 섹션에 위치 — 뒤에 다른 하위 섹션을 추가하지 말 것. 공시 내용이 실제
데이터와 어긋나면 시스템이 이 섹션만 자동 제거하므로, 이 섹션이 맨 뒤에 있어야 다른 섹션이 함께 삭제되지 않는다.)
...
첫 문장은 오늘 하루를 관통하는 해석 한 가지를 저자 관점으로 짚을 것 — 표면
등락률 나열이 아니라 위 1)~2) 섹션에서 이미 다룬 수치로는 안 보이는 구조적
특징(예: "지수는 급등했지만 업종폭은 소수 종목 견인이었다", "상승·하락 특징주
등락폭 격차가 유독 컸다", "특정 섹터로의 쏠림이 다른 섹터 대비 두드러졌다")을
오늘 데이터(업종폭·특징주 격차·섹터 수)에서 직접 도출해 서술. 확신형 단정
대신 "~로 읽힙니다", "~라는 인상입니다" 같은 해석형 어조 사용 가능.
이어서 오늘 섹터·특징주 흐름을 근거로 1~2문장 추가. 과도한 낙관·비관 금지.
...
⚠️ 문단 골격을 매번 똑같이 반복하지 말 것 — 첫 문장 해석의 소재를 매번 바꿀 것
(어떤 날은 업종폭, 어떤 날은 특징주 등락폭 격차, 어떤 날은 상승·하락 섹터 수
비교 등으로 번갈아 사용 — 매일 같은 관점("업종폭")만 반복하지 말 것).
체크포인트 문장의 표현도 매번 다르게 쓸 것("~인지 지켜볼 필요가 있습니다",
"~여부가 관건입니다", "~에 주목할 필요가 있습니다" 등을 돌아가며 사용 — 동일 어구 반복 금지).
'내일' 대신 반드시 '{next_day_label}' 표현 사용.
전체 3~4문장 이내로 유지 — 해석 1문장 + 전망 근거 1~2문장 + 체크포인트 1문장.
