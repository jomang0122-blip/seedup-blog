...
def _build_stock_anchor(data: dict) -> str:
    ...
    if gainers:
        ...
        lines.append("상승 특징주 (이유 작성 절대 금지 — [관련뉴스]가 있으면 원문 그대로만 병기):\n" + "\n".join(parts))

    if losers:
        parts = []
        for s in losers:
            parts.append(f"{s['name']} {s['change_pct']:+.2f}%{_news_tag(s['name'])}")
        lines.append("하락 특징주 (이유 작성 절대 금지 — [관련뉴스]가 있으면 원문 그대로만 병기):\n" + "\n".join(parts))
...
def _build_sector_anchor(data: dict) -> str:
    ...
    def _sector_line(s):
        ...
        if verdict == "isolated":
            ...
            base += " [혼재 — 같은 방향이 과반 근접. '동반'도 '개별 견인'도 단정 금지, 업종폭 수치 인용 권장]"
...
6. 특징주 섹션별 이유 작성 규칙:
   - 📈 상승 특징주 / 📉 하락 특징주: 이유 작성 절대 금지. 종목명과 등락률만 표기.
     ...
     단, 데이터 블록에 [관련뉴스: ...]가 붙은 종목은 그 헤드라인을 "(관련뉴스: 원문 그대로)"
     형태로 등락률 뒤에 병기할 것. 헤드라인을 요약·재해석·이유 문장으로 변환 금지,
     원문 그대로만 붙일 것. 이 헤드라인을 도입부·전망 등 다른 섹션의 서술 근거로
     재사용하는 것도 금지 (병기 전용 참고 자료).
   ...
   - [당일 주도 섹터 및 테마] 서술 규칙 — 데이터 블록의 [업종폭] 수치와 표시가 판정 근거:
     - ⚠️[소수 종목 견인] 표시: 구성종목 과반이 섹터와 반대/보합인데 소수 종목이 지수를
       움직인 것. "동반 강세/약세", "업종 전반" 표현 금지. "OO 종목이 섹터 등락률을
       견인" 처럼 견인 종목과 섹터 전체를 분리해서 서술할 것.
     - [업종 전반 흐름] 표시: 구성종목 60% 이상 같은 방향 — "업종 전반 강세/약세" 서술
       가능. 굳이 "불확실" 같은 유보 표현을 붙이지 말 것.
     - [혼재] 표시: 같은 방향이 과반 근접 수준 — "동반 강세"도 "개별 종목 견인"도 단정
       하지 말고, 업종폭 수치를 인용해 사실대로 서술할 것
       (예: "금호타이어 상한가 속 업종 내 등락 혼재(151종목 중 77개 상승)").
     - 아무 표시 없는 섹터: 업종폭 데이터 없음 — 전반/개별 어느 쪽도 단정하지 말 것.
     - [업종폭] 수치(예: "34종목 중 24개 동방향")는 서술의 근거로 자연스럽게 활용 가능.
       단 반드시 그 섹터 자신의 [업종폭] 값만 그대로 사용 — 다른 섹터 행의 수치를
       옮겨 적지 말 것 (작성 후 각 행의 수치가 해당 섹터 데이터와 일치하는지 재확인).
     - [대표종목 뉴스]가 있으면 "핵심 흐름 한 줄"의 근거로 사용 가능 — 단 원문 취지
       그대로만, 확대해석·인과 창작 금지. 뉴스 없는 섹터는 업종폭·등락률 사실만으로 서술.
   - 공통 금지 사항:
...
#### 📈 상승 특징주
- **[종목A]** <span style="color:#e74c3c"><b>+X.XX%</b></span> (관련뉴스: [데이터 블록의 관련뉴스 원문 그대로 — 없으면 괄호 자체 생략])
- **[종목B]** <span style="color:#e74c3c"><b>+X.XX%</b></span>

#### 📉 하락 특징주
- **[종목X]** <span style="color:#3182f6"><b>-X.XX%</b></span> (관련뉴스: [원문 그대로 — 없으면 생략])
