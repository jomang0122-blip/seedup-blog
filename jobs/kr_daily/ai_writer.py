...
def _build_dart_anchor(data: dict) -> str:
    ...
    사실관계 나열만 허용(해석·이유 서술 금지) — 회사명·보고서명·접수일이
    실제 DART 응답 값과 일치해야 하므로 AI 해석을 허용하면 환각 위험이 생긴다."""
    disclosures = data.get("dart_disclosures", {})
    if not disclosures:
        return "(공시 없음 — #### 📋 관련 공시 소제목 포함 해당 하위 섹션 전체 생략. 텍스트 한 줄도 출력 금지.)"
    lines = [
        f"{name} | {info['report_nm']} | 접수 {info['rcept_dt']} | {info['url']}"
        for name, info in disclosures.items()
    ]
    return "이 목록에 있는 종목·보고서명·접수일만 사용 — 이유·해석 문장 작성 금지:\n" + "\n".join(lines)
...
6. 특징주 섹션별 이유 작성 규칙:
   ...
   - 📋 관련 공시: 데이터 블록 'DART' 목록에 있는 종목·보고서명·접수일·URL만 사용.
     ...
     형식의 마크다운 링크로만 표기하고 이유·해석 문장 작성 절대 금지 — 사실 나열만.
     URL은 데이터 블록에 있는 값을 한 글자도 바꾸지 말 것. 목록이 없으면 이 하위
     섹션 전체 생략.
   - 공통 금지 사항:
