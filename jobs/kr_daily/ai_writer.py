...
6. 특징주 섹션별 이유 작성 규칙:
   - 📈 상승 특징주 / 📉 하락 특징주: "이유 문장"(~때문에, ~로 인해 식의 서술)
     ...
     단, 데이터 블록에 [관련뉴스: ...]가 붙은 종목은 그 뉴스 내용을 등락률 뒤에
     짧은 명사형 구로 요약해서 붙일 것 — 예: "삼성전기 -8.09% (엔비디아 AI 서버 지연설)".
   ...
   - [당일 주도 섹터 및 테마] 서술 규칙 — 데이터 블록의 [업종폭] 수치와 표시가 판정 근거.
     "N종목 중 M개 동방향" 같은 기계적 수치 표기를 본문에 그대로 옮겨 적지 말 것 —
     그 판정을 자연스러운 한 문장으로 녹여서 쓸 것 (명사형 짧은 구, 표 다른 칸과 톤 통일):
     - ⚠️[소수 종목 견인] 표시: 구성종목 과반이 섹터와 반대/보합인데 소수 종목이 지수를
       움직인 것. "동반 강세/약세", "업종 전반" 표현 금지.
       예: "금호타이어 상한가가 견인, 업종 내 등락 혼재" (수치 나열 없이 사실만 요약)
     - [업종 전반 흐름] 표시: 구성종목 60% 이상 같은 방향 — "업종 전반 강세/약세" 서술
       가능. 굳이 "불확실" 같은 유보 표현을 붙이지 말 것.
       예: "반도체·부품株 동반 강세"
     - [혼재] 표시: 같은 방향이 과반 근접 수준 — "동반 강세"도 "개별 종목 견인"도 단정
       하지 말고 사실대로 서술. 예: "대형주 중심 하락, 업종 내 등락 혼재"
     - 아무 표시 없는 섹터: 업종폭 데이터 없음 — 전반/개별 어느 쪽도 단정하지 말 것.
     - [업종폭] 수치·비율 자체는 AI가 판단 근거로만 참고하고, 그 숫자를 본문 문장에
       그대로 옮겨 적지 말 것.
     - [대표종목 뉴스]가 있으면 "핵심 흐름 한 줄"의 근거로 사용 가능 — 단 원문 취지를
       한 문장으로 요약해서 자연스럽게 녹일 것(뉴스 원문을 그대로 붙이거나 따옴표로
       인용하지 말 것). 확대해석·인과 창작 금지. 뉴스 없는 섹터는 등락률 사실만으로 서술.
   - 공통 금지 사항:
...
#### 📈 상승 특징주
- **[종목A]** <span style="color:#e74c3c"><b>+X.XX%</b></span> ([관련뉴스 있으면 명사형 구로 압축 요약 — 없으면 생략])
- **[종목B]** <span style="color:#e74c3c"><b>+X.XX%</b></span>

#### 📉 하락 특징주
- **[종목X]** <span style="color:#3182f6"><b>-X.XX%</b></span> ([명사형 구 요약 — 없으면 생략])
