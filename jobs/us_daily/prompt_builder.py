...
9. **마크다운 금지**: HTML 출력이므로 `**bold**` 등 마크다운 기호 절대 사용 금지. 두꺼운 글씨는 반드시 `<strong>텍스트</strong>` HTML 태그 사용.
10. HTML 테이블 헤더: 첫 번째 행(컬럼명)은 반드시 `<th>` 태그 사용. 예: `<tr><th>지수명</th><th>종가</th><th>등락률</th></tr>`
...
b) <h3>🔥 오늘의 주목 종목</h3>
   - HTML 테이블: 종목 | 종가 | 등락률 | 한줄 동향
     * 종목 컬럼: 반드시 <strong>TICKER(한글명)</strong> 형식으로 표기 (예: <strong>NVDA(엔비디아)</strong>)
...
출력 형식 — 아래 헤더 뒤에 HTML 본문만 작성 (면책 조항은 포함하지 말 것. 시스템이 자동 추가):
...
def _md_to_html(text: str) -> str:
    """마크다운 기호(**, -, 표 등)를 HTML로 변환. 기존 HTML 태그는 유지."""
    try:
