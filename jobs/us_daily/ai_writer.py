...
from shared.utils import DISCLAIMER, US_REPORT_LINKS_HTML, md_to_html, apply_color_spans, fix_weekday_labels, us_time_rule_block, truncate_at_sentence, extract_text
...
def build_prompt(data: dict) -> str:
    ...
    if has_economic:
        ...
        economic_skip_note = (
            ...
            "  향후 일정(FOMC 등) 언급으로 대체하는 것도 금지 — 데이터에 없는 날짜를 추측하게 되므로 절대 금지."
        )
...
11. 문체 규칙 (기계적 반복 금지):
    ...
    - '차익실현'(관련 표현 포함)은 하락 이유로 글 전체에서 최대 1회까지만 사용할 것 —
      뉴스 근거 없는 하락 종목·섹터를 설명해야 할 때마다 만능 이유로 반복 사용하지
      말 것. 데이터·뉴스에서 찾을 수 있는 다른 근거(섹터 순환매, 밸류에이션 부담,
      직전 급등에 따른 되돌림 등)가 있으면 그것을 우선 쓰고, 그런 근거조차 전혀
      없으면 이유 서술 자체를 생략할 것(규칙 1의 환각 금지 원칙 그대로 적용).
...
[아래 0) "📌 오늘 미국증시 핵심" 섹션 작성 지침 — 이 지침 설명 텍스트 자체는 절대
본문에 출력하지 말 것. 실제로 출력할 것은 오직 "📌 **오늘 미국증시 핵심**" 다음에
이어지는 요약 문장뿐이다.]
- 이 섹션은 구글 검색 결과 설명(meta description)으로 그대로 노출된다.
- 날짜·나스닥·S&P500·다우 등락률·핵심 종목 키워드를 첫 2문장 안에 반드시 포함.
- 이모티콘 제외 150자 이내로 압축.

구조 (반드시 이 순서로):
0) 📌 **오늘 미국증시 핵심** [요약 문장 — 위 지침만 따르고 괄호 설명은 출력 금지]
...
b) ### 🔥 오늘의 주목 종목
   ...
   - 데이터 블록 '급등락 종목' 목록의 종목마다 한 줄씩만 출력 (전체 나열 줄과
     ...
     * 뉴스기반 급등락 목록에 없으면:
       📈 TICKER(한글명) +X.X%  (상승) / 📉 TICKER(한글명) -X.X%  (하락)
       (⚠️ 이 경우 이유 문구를 절대 추가하지 말 것 — 위 형식 그대로만 출력, 이 괄호 설명 자체도 출력 금지)
...
c) ### 📋 경제 지표 & 연준 동향
   ...
   - ⚠️ 이 섹션에는 "오늘 발표된 것"만 서술. 앞으로 발표될 경제지표·FOMC·실적 일정은
     이 섹션을 포함해 본문 어디에도 절대 서술 금지 — 데이터 블록에는 향후 일정 정보가
     전혀 없어 AI 지식으로 날짜를 추측할 수밖에 없고, 그 결과 실제 사고가 반복됐다
     (이미 지난 FOMC 의사록을 미래 일정으로 안내, CPI 발표일 오기재가 3주 연속 발생).
     "다음 주요 일정" 같은 구체적 날짜 예고 문구 자체를 쓰지 말 것.
   ...
   - "일부 전략가들은", "일부 시장 전문가들은", "시장에서는", "~라는 시각이 있습니다",
     ...
     섹션도 짧게(1문장) 끝낼 것
   - 당일 발표 내용이 전혀 없으면 ### 📋 경제 지표 & 연준 동향 섹션 전체 생략
...
d) ### 📰 오늘의 핵심 뉴스
   ...
   - ⚠️ 이 섹션이 마지막 섹션 — "내일 일정", "다음 주요 일정" 등 향후 일정 섹션·문장을
     본문 어디에도 추가 금지 (데이터 블록에 향후 일정 정보가 없어 날짜 추측으로
     이어지는 것을 원천 차단하기 위함)
