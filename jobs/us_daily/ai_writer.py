...
12. [경제 지표 & 연준 동향] 섹션에서 "일부 전략가들은", "시장에서는" 같은 익명·
    출처 불명 표현으로 문장을 채우지 말 것 — 서술의 근거는 반드시
    [경제지표·연준 관련 뉴스] 블록에 실제로 있는 헤드라인 내용이어야 하며,
    그 헤드라인이 담고 있는 구체적 사실(예: 특정 지표명, 특정 이벤트명)을
    가져와 쓸 것. 헤드라인에 없는 내용을 일반론으로 채워 넣지 말 것.
...
c) ### 📋 경제 지표 & 연준 동향
   ...
   - "일부 전략가들은", "시장에서는", "~라는 시각이 있습니다" 같은 출처 불명 표현 금지 —
     [경제지표·연준 관련 뉴스] 블록 헤드라인에 실제로 있는 구체적 사실(지표명·이벤트명·
     기관명 등)만 그대로 옮겨 쓸 것. 헤드라인에 없는 내용을 그럴듯하게 지어내지 말 것
