# -*- coding: utf-8 -*-
"""
SeedUP 국내증시 월간 결산 자동 발행 파이프라인
실행: python jobs/kr_monthly/main.py [--dry-run] [--force]
"""
import argparse
import io
import json
...
def build_title(data: dict) -> str:
    ...
    pct = kospi.get("monthly_pct", 0) or 0
    direction = "상승" if pct >= 0 else "하락"
    return f"[{month_label} 국내증시 결산] 코스피 {abs(pct):.1f}% {direction} 월간 리뷰"


def save_log(data: dict, post: dict, result: dict, kst_date: str, validation_issues: list = None):
    log_file = LOG_DIR / f"kr_monthly_{kst_date.replace('-', '')}.json"
    record = {
        "kst_date": kst_date,
        ...
        "kospi_monthly_pct": data.get("kospi", {}).get("monthly_pct"),
...
def run(dry_run: bool = False, force: bool = False):
    ...
    log("=" * 50)
    log(f"SeedUP 국내증시 월간 결산  {'[DRY-RUN]' if dry_run else '[LIVE]'}  KST {kst_now.strftime('%Y-%m-%d %H:%M')}")
    log("=" * 50)

    # 매월 1일에만 발행 — force 시 생략
    if not force and kst_now.day != 1:
        log(f"  오늘은 {kst_now.day}일 — 매월 1일에만 발행합니다. 종료.")
        sys.exit(0)

    log("▶ Step 1: 국내 월간 결산 데이터 수집")
    try:
        data = collect_all()
    ...
    kospi = data.get("kospi", {})
    log(f"  결산 대상: {data['month_label']} ({data['month_start']} ~ {data['month_end']})")
    log(f"  KOSPI: {kospi.get('close', 'N/A')} (월간 {kospi.get('monthly_pct', 0):+.2f}%)")
    ...
    if force:
        ...
        try:
            existing = check_today_post(kst_date, label_filter="국내증시 결산]")
            if existing:
    ...
    try:
        result = publish_post(
            ...
            labels=post.get("labels", ["국내증시", "월간결산", "코스피"]),
            status="LIVE",
        )
        log(f"  발행 완료!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SeedUP 국내증시 월간 결산 자동 발행")
