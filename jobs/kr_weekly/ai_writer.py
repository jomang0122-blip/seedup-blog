...
""" if has_market_trend else ""
    ...
    stocks_skip_note = (
        "\n⚠️ 급등락 종목 데이터 없음 → 아래 c) 이번 주 급등락 종목 섹션 전체 생략. 소제목 포함 텍스트 한 줄도 출력 금지. 종목명 임의 생성 절대 금지."
...
1-1. 수급 금액은 반드시 제공된 억원 단위 그대로 사용. 조원 단위 변환·총액 합산·임의 계산 절대 금지.
...
0) 📌 **이번 주 핵심 요약** — 아래 4가지를 3~4문장(이모티콘 제외 200~280자)으로 작성:
   ...
   - 문장 4: 다음 주 핵심 변수 한 줄 예고 (FOMC, 실적 발표 등)
