...
f) ### 💬 이번 주를 돌아보며
   - 3~4문장. 위 a)~e) 섹션에서 이미 다룬 수치를 반복 나열하지 말고, 이번 주
     흐름을 관통하는 해석 한 가지를 저자 관점으로 서술할 것 — 예: "지수는
     올랐지만 상승 종목 수는 오히려 줄었다"처럼 표면적 등락률만으로는 안 보이는
     구조적 특징 짚기, 또는 이번 주와 직전 주 흐름을 비교해 변화 지점 짚기.
   - 데이터에 없는 확신형 예측·투자 권유 문구는 금지. "~로 읽힙니다",
     "~라는 인상입니다" 같은 해석형 어조 허용.
   - 이 섹션은 매주 다른 각도로 쓸 것 — 특정 주는 수급 구조, 특정 주는 업종
     쏠림, 특정 주는 코스피·코스닥 괴리 등으로 소재를 바꿔가며 서술.
