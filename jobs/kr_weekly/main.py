...
from ai_writer import generate_post, KR_WEEKLY_REQUIRED_SECTIONS
...
def run(dry_run: bool = False, force: bool = False):
    ...
    for attempt in range(3):
        ...
        try:
            assert_structure_complete(candidate["content"], KR_WEEKLY_REQUIRED_SECTIONS, "국내증시 위클리")
        except ValueError as e:
            log(f"  [경고] {e}")
            if attempt < 2:
        ...
        # kr_daily 실사고로 확인된 동일 계열 버그, kr_weekly도 같은 로직이라 함께 수정).
