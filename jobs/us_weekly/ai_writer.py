...
from shared.utils import DISCLAIMER, US_REPORT_LINKS_HTML, md_to_html, apply_color_spans, fix_weekday_labels, truncate_at_sentence, extract_text
...
US_WEEKLY_REQUIRED_SECTIONS = [
    ...
    ("한국인 관심 종목 주간 성적",  "🔥 한국인 관심 종목 주간 성적"),
    ("주간 급등락 TOP3",         "💥 주간 급등락 TOP3"),
...
[아래 0) "📌 이번 주 미국증시 핵심" 섹션 작성 지침 — 이 지침 설명 텍스트 자체는
절대 본문에 출력하지 말 것. 실제로 출력할 것은 오직 "📌 **이번 주 미국증시 핵심**"
다음에 이어지는 요약 문장뿐이다.]
- 이 섹션은 구글 검색 결과 설명(meta description)으로 그대로 노출된다.
- 주간 나스닥·S&P500·다우 등락률·핵심 이슈를 첫 2문장 안에 반드시 포함.
- 이모티콘 제외 150자 이내로 압축.

구조 (반드시 이 순서로):
0) 📌 **이번 주 미국증시 핵심** [요약 문장 — 위 지침만 따르고 괄호 설명은 출력 금지]
...
d) ### 💬 이번 주를 돌아보며
   - 3~4문장. 위 a)~c) 섹션에서 이미 다룬 수치를 반복 나열하지 말고, 이번 주
     흐름을 관통하는 해석 한 가지를 저자 관점으로 서술할 것 — 예: "지수 상승
     대비 상승 종목 폭이 좁았다"처럼 표면적 등락률만으로는 안 보이는 구조적
     특징 짚기, 또는 이번 주와 직전 주 흐름을 비교해 변화 지점 짚기.
   - 데이터에 없는 확신형 예측·투자 권유 문구는 금지. "~로 읽힙니다",
     "~라는 인상입니다" 같은 해석형 어조 허용.
   - 이 섹션은 매주 다른 각도로 쓸 것 — 특정 주는 수급 구조, 특정 주는 업종
     쏠림, 특정 주는 대형주·중소형주 괴리 등으로 소재를 바꿔가며 서술.
...
LABELS: {all_labels}
SEARCH_DESC: 구글 검색결과 설명(meta description)으로 그대로 쓰일 문장 — 본문의 "📌 이번 주 미국증시 핵심"과
  같은 소재를 다루되, 반드시 이모티콘 제외 110자 이내로 더 짧게 압축한 별도 문장. 주간 나스닥·S&P500·다우
  등락률·핵심 이슈 키워드 포함. HTML 태그·이모티콘·따옴표 없는 순수 텍스트 한 문단.
CONTENT:
...
def _parse_response(raw: str, ref_date: str = "") -> dict:
    ...
    # ⚠️ 2026-09-01 수정: search_description(SEO 메타디스크립션)을 "🔍 이번 주
    # 시장 요약"이라는 이름으로 본문에 그대로 노출시키고 있었는데, 이미 위에서
    # 다룬 내용을 끝에 한 번 더 반복하는 어색한 섹션이 됐다. kr_daily·kr_monthly는
    # 이 값을 본문에 넣지 않고 로그로만 남겨 총괄이 Blogger 편집화면에 수동으로
    # 붙여넣게 하는데(searchDescription은 Blogger API로 설정 불가, 2026-08-26
    # 확인됨), us_daily·us_weekly만 다르게 동작하고 있었다. 같은 방식으로 통일.
