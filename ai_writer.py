...
def _build_prompt(data: dict) -> str:
    ...
    def stock_lines_with_news(stocks):
        ...
        for i, s in enumerate(stocks):
            line = f"  {i+1}. {s['name']}  {s['change_pct']:+.2f}%"
...
특징주 급등 TOP5 (KOSPI 전체 종목 기준):
{gainers_txt}

특징주 급락 TOP5 (KOSPI 전체 종목 기준):
{losers_txt}
...
3. 구조 (반드시 이 순서로):
   ...
   e) <h3>🔥 특징주 리포트</h3> — "KOSPI 전체 종목 기준" 한 줄 명시 후, 급등·급락 각각 <ul><li> 리스트. 종목마다 등락률 + 뉴스/이유 한 줄
...
D. 환각 금지 (가장 중요): 종목 이유는 반드시 위 "관련뉴스" 항목에 있는 내용만 사용할 것.
   - 관련뉴스가 있는 종목 → 뉴스 기반으로 한 줄 이유 작성
   - 관련뉴스가 없는 종목 → 이유 없이 종목명과 수치만 표기 (이유를 지어내지 말 것)
   - 예시(뉴스 없음): <li>SK오션플랜트 +17.25%</li>
   - 예시(뉴스 있음): <li>SK하이닉스 +13.06% — 마이크론 실적 호조로 반도체 업종 전반 수혜</li>
   - 절대 금지: 관련 없는 정보를 억지로 연결하거나 그럴듯한 이유를 창작하는 것
